...
pub struct RedirectRules {
    pub model: Vec<ModelRedirectRule>,
}
...
#[cfg(test)]
fn get_test_model_redirect_rule() -> ModelRedirectRule {
    ModelRedirectRule {
...
fn test_redirect_deserialize() {
    ...
    let redirect = RedirectRules {
        model: vec![get_test_model_redirect_rule()],
    };

    let result: RedirectRules = serde_xml_rs::from_str(xml).unwrap();
